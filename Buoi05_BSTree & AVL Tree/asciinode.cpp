...
asciinode* build_ascii_tree_recursiveBooks(AVLBOOK* t)
{
	...
	node = new asciinode;
	node->left = build_ascii_tree_recursiveBooks(t->Left);
	node->right = build_ascii_tree_recursiveBooks(t->Right);
	...
	string str = t->Data.Tensach;
	char x[35];
	str.copy(x, str.length());
	int a = str.length();
	x[a] = '\0';
	
	sprintf_s(node->label, "%s", x);
	node->lablen = strlen(node->label);
	return node;
}
...
asciinode* build_ascii_tree_recursiveDct(AVLDictNode* t)
{
	...
	node = new asciinode;
	node->left = build_ascii_tree_recursiveDct(t->Left);
	node->right = build_ascii_tree_recursiveDct(t->Right);
...
asciinode* build_ascii_treeBooks(AVLBOOK* t)
{
	asciinode* node;
	if (t == NULL) return NULL;
	node = build_ascii_tree_recursiveBooks(t);
	node->parent_dir = 0;
	return node;
}
asciinode* build_ascii_treeDct(AVLDictNode* t)
{
	asciinode* node;
	if (t == NULL) return NULL;
	node = build_ascii_tree_recursiveDct(t);
	node->parent_dir = 0;
	return node;
}
